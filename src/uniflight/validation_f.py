"""Milestone F/F.1 reference landing scenario used for validation and benchmarks.
...
from .control import LandingGNCController, QuaternionPDController, VectorLandingGuidance
...
class FLandingResult:
    result: ClosedLoopResult
    metrics: Mapping[str, float | bool]


def build_f_landing_case(*, seed: int, lateral_y: float = 10.0, lateral_z: float = 0.0,
...
def run_f_landing(*, seed: int = 7, sample_period: float = 0.5,
                  integrator_kind: str = "scipy", rk4_step: float = 0.1,
                  record_trajectory: bool = True, record_gnc_records: bool | None = None,
                  **kwargs) -> FLandingResult:
    body, schema, y0, sim, touchdown = build_f_landing_case(
    ...
    return FLandingResult(result, metrics)


def f_landing_monte_carlo_case(params: Mapping[str, float], rng: np.random.Generator,
